...
public class Tools {
    ...
    public static void launchPingpongupdate(Activity activity){
        if(activity == null){
            return;
        }

        Intent pingpongupdate = new Intent(Intent.ACTION_VIEW);
        pingpongupdate.setData(Uri.parse(activity.getString(R.string.pingpong_link)));
        if(pingpongupdate.resolveActivity(activity.getPackageManager()) != null){
            activity.startActivity(pingpongupdate);
        }else{
            new ToastMsg(activity).toastIconError("No App found");
        }
